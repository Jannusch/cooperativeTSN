...
# change the default OMNeT++ workspace path to a path inside the devcontainer's workspace
sed -i 's/DEFAULT_WORKSPACE_ARGS="-vmargs -Dosgi\.instance\.area\.default=\$IDEDIR\/\.\.\/samples"/DEFAULT_WORKSPACE_ARGS="-vmargs -Dosgi\.instance\.area\.default=\/workspaces\/nsm-courses-devcontainer\/omnetpp\.workspace"/g' /opt/omnetpp/bin/opp_ide
