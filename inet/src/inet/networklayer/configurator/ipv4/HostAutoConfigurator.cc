...
// Copyrigth (C) 2024 Jannusch Bigge <jannusch.bigge@tu-dresden.de>
//
...
void HostAutoConfigurator::setupNetworkLayer()
{
    ...
    while ((ifname = interfaceTokenizer.nextToken()) != nullptr) {
        ...
        // generate address out of base and interface module id
        Ipv4Address interfaceAddress = Ipv4Address(addressBase.getInt() + uint32_t(ie->getId()));

        // address test
        if (!Ipv4Address::maskedAddrAreEqual(interfaceAddress, addressBase, netmask))
            throw cRuntimeError("Generated IP address is out of specified address range");
        ...
        EV_INFO << "interface " << ifname << " gets " << interfaceAddress.str() << "/" << netmask.str() << std::endl;

        ipv4Data->setIPAddress(interfaceAddress);
