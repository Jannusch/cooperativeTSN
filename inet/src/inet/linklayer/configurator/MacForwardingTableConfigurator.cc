...
void MacForwardingTableConfigurator::computeMacForwardingTables()
{
    ...
    // setting the values hardcoded? ich bin mir nicht sicher...
}
...
{
    // maybe this is the correct place to adjust the network topology  ?
    // I'm not sure...
    // I actualy think that this is the best place to do it
    // It allows me to adjust the topology, add default nodes, add external nodes and so on


    // First I have to check whether the destination node is in the actual subnetwork
    // Otherwise the following algorithm will not work due to nullpointers
    // auto parent_node = getContainingNode(this)->getIndex();
