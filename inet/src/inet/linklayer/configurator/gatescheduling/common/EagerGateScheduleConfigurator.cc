        ...
        for (auto pathFragment : flow.pathFragments) {
            if (!strcmp(pathFragment->networkNodes.front()->module->getFullName(), std::get<0>(todo).c_str())) {
                ...
                for (int i = 0; i < pathFragment->networkNodes.size() - 1; i++) {
                    ...
                    // check if module is a scalar type or vector type
                    cModule* networkNode;
                    if(pathFragment->networkNodes[i]->module->isVector()){
                        auto name = std::string(networkNodeName).substr(0, std::string(networkNodeName).find("["));
                        networkNode = getParentModule()->getSubmodule(name.c_str(), pathFragment->networkNodes[i]->module->getIndex());
                    } else {
                        networkNode = getParentModule()->getSubmodule(networkNodeName);
                    }
