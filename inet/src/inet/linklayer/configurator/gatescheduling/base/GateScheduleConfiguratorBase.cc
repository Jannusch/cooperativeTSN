    ...
    for (int i = 0; i < topology->getNumNodes(); i++) {
        ...
        for (auto interface : node->interfaces) {
            ...
            if (!networkInterface->isLoopback() && !networkInterface->isWireless()) {
                ...
                // TODO: This is juts wrong... the delay is the time the packet is on the wire depending on the length of the physical medium. Not the delay that is dependent of the Datarate
                ...
                port->cutthroughSwitchingEnabled = false; // TODO: extract from network interface!
...
void GateScheduleConfiguratorBase::configureGateScheduling(cModule *networkNode, cModule *gate, Interface *interface)
{
    ...
    // TODO: Why was this usefull?
    if (durations->size() == 0){
        EV_INFO << "Empty TSN schedule. Using values from omnetpp.ini" << EV_ENDL;
        return;
    }
...
void GateScheduleConfiguratorBase::configureApplicationOffsets()
{
    for (auto& it : gateSchedulingOutput->applicationStartTimes) {
        ...
        if(par("configureApplicationOffsets").boolValue()){
